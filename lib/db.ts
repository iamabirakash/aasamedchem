...
if (!process.env.DATABASE_URL && process.env.NODE_ENV !== "production") {
  console.warn("DATABASE_URL is not set. Database-backed pages require a Neon connection.");
}
...
export const sql: any = ((strings: TemplateStringsArray, ...values: unknown[]) => {
...
export async function ensureSchema() {
  if (schemaReady || !process.env.DATABASE_URL) {
    return;
  }
